...
public interface ComprobanteRepository extends JpaRepository<Comprobante, Integer> {
    ...
    @Query(value = "select  id_comprobante,comp_serie_correlativo,recep_numero_documento, " +
            "estado, fecha, usuario_creacion from ( select *,COUNT(*) OVER(PARTITION BY comp_serie_correlativo ) C FROM comprobante )" +
            " as Comp WHERE C >1", nativeQuery = true)
    List<Comprobante>countTotalComprobanteByRepetido();
